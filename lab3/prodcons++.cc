...
//   page 182 of the textbook.
    ...
    for (num = 0; num < N_MESSG ; num++) {
      ...
      slot *message = new slot(which,num);


      // Put the code for synchronization before  ring->Put(message) here.
      nfull->P(); // Determine the ring weather is full
      mutex->P(); // Acquire the lock to put
...
Consumer(_int which)
{
    ...
    for (; ; ) {
      ...
      nempty->P(); // If the ring is empty,then wait
      mutex->P(); // Acquire the lock to get.
...
ProdCons()
{
    ...
    nfull = new Semaphore("full",BUFF_SIZE);
    nempty = new Semaphore("empty",0);
